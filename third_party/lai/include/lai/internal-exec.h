...
__attribute__((always_inline)) inline void lai_exec_pkg_load(lai_variable_t* out, lai_variable_t* pkg, size_t i)
{
    LAI_ENSURE(pkg->type == LAI_PACKAGE);
    lai_exec_pkg_var_load(out, pkg->pkg_ptr, i);
}
__attribute__((always_inline)) inline void lai_exec_pkg_store(lai_variable_t* in, lai_variable_t* pkg, size_t i)
{
    LAI_ENSURE(pkg->type == LAI_PACKAGE);
