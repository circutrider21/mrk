...
static fs::node *path2node(fs::node *parent, const char *_path, int flags) {
    bool last = false;
    if (_path == nullptr)
        return nullptr;
...
next:;
    ...
    for (;;) {
        ...
        if (_strcmp(cur_node->name, elem) != 0) {
            // If match found
            if (mount_peek)
        ...
        if (last) {
            if (flags & NODE_FAIL_IF_EXISTS) {
                return nullptr;
            }

            if (cur_node->res != nullptr && S_ISLNK(cur_node->res->st_mode)) {
                if (flags & NODE_NO_DEREF_LINKS) {
                    return nullptr;
                }
...
epilogue:
    if (flags & (NODE_CREATE_SHALLOW | NODE_CREATE_DEEP)) {
        if (last) {
            return fs::create_node(cur_parent, elem);
        } else {
            if (!(flags & NODE_CREATE_DEEP)) {
                return nullptr;
            }
...
fs::node *fs::mkdir(fs::node *parent, const char *name, uint32_t mode, bool recurse) {
    if (parent == nullptr)
        parent = fs_root;
    ...
    new_dir = path2node(parent, name, recurse ? NODE_CREATE_DEEP : NODE_CREATE_SHALLOW);
    ...
    fs::node *dot = fs::create_node(new_dir, ".");
    dot->redir = new_dir;

    fs::node *dotdot = fs::create_node(new_dir, "..");
...
bool fs::mount(const char* source, const char* target, const char* fs_type)
{
    ...
    if (mounting_root) {
        ...
        _strcpy(tgt_node->name, mount_gate->name);
    ...
    log("fs: `%s` is now mounted as '%s'. (type: `%s`)\n", fs->is_backed ? source : "<no-device>", target, fs_type);
